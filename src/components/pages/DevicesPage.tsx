                ...
                <div className="flex justify-between pt-4 border-t">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      onClick={async () => {
                        if (!window.confirm("Reset API Key cho thiết bị này?"))
                          return;
                        const res = await axios.post(
                          `http://localhost:5000/api/devices/${device.id}/reset-key`
                        );
                        alert("API Key mới: " + res.data.newKey);
                        fetchDevices();
                      }}
                      title="Reset API Key"
                    >
                  ...
                  >
                    ...
                    <TableCell className="px-6 space-x-2 text-left">
                      ...
                      <Button
                        size="sm"
                        variant="outline"
                        className="rounded-full text-purple-600 border-purple-300 hover:bg-purple-50"
                        onClick={async () => {
                          const res = await axios.post(
                            `http://localhost:5000/api/devices/${d.id}/reset-key`
                          );
                          alert("API Key mới: " + res.data.newKey);
                        }}
                      >
    ...
    <div>
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          {/* Quận */}
          <div>
            <label className="block text-xs text-gray-600 mb-1.5 font-medium">Quận/Huyện</label>
            <select
          ...
          {/* Xã */}
          <div>
            <label className="block text-xs text-gray-600 mb-1.5 font-medium">Xã/Phường</label>
            <select
        ...
        {/* Mô tả */}
        <div>
          <label className="block text-xs text-gray-600 mb-1.5 font-medium">Mô tả</label>
          <textarea
            rows={2}
  ...
  {selectedDevice && (
    <div className="space-y-6">
      ...
      <div className="p-4 bg-gray-50 rounded-xl border">
        ...
        <p className="font-mono text-gray-900 text-sm">{selectedDevice.device_id}</p>
      </div>
      ...
      <div className="p-4 bg-gray-50 rounded-xl border space-y-2">
        ...
        <input
          readOnly
          value={getUploadUrl(selectedDevice.device_id)}
          className="w-full bg-white border px-3 py-2 rounded-lg font-mono text-sm"
        />
      </div>
