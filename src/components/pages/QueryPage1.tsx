...
import { Search, Filter, Calendar, MapPin, HardDrive, Download, FileText, Layers, History, RefreshCcw } from "lucide-react";
...
export default function DataQuery() {
  ...
  const fetchMeta = async () => {
    ...
    } catch (e) { console.error("Lỗi tải metadata", e); }
  };
...
const foundProperties = new Set<string>();
    ...
    const processedRows = Array.from(groupedMap.values()).sort((a, b) => 
  ...
  const handleExportCSV = async () => {
    ...
    // --- A. Tạo file CSV ---
    const headers = ["Thời gian", "Thiết bị", "Vị trí chi tiết", ...dynamicColumns, "Nhãn cảnh báo"];
...
const fileName = `Bao_cao_IoT_${dates.from}_${dates.to}.csv`;
    ...
    try {
        await axios.post(`${API_BASE}/data-query/history?fileName=${fileName}`, createPayload(), {
            headers: { Authorization: `Bearer ${localStorage.getItem("token")}` }
        });
  ...
  return (
    <div className="space-y-6 p-4">
      ...
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="text-gray-500">Tìm kiếm dữ liệu lịch sử lưu trữ trên MinIO theo Loại thiết bị & Vị trí</p>
      </motion.div>
...
<label className="text-xs text-gray-500">Từ ngày</label>
              <input type="date" className="w-full border rounded p-2 text-sm" 
                value={dates.from} onChange={e => setDates({...dates, from: e.target.value})} />
            </div>
            <div>
              <label className="text-xs text-gray-500">Đến ngày</label>
              <input type="date" className="w-full border rounded p-2 text-sm"
                value={dates.to} onChange={e => setDates({...dates, to: e.target.value})} />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-2">
            <div>
              ...
              <input type="time" className="w-full border rounded p-2 text-sm"
                value={times.from} onChange={e => setTimes({...times, from: e.target.value})} />
            </div>
            <div>
              <label className="text-xs text-gray-500">Đến giờ</label>
              <input type="time" className="w-full border rounded p-2 text-sm"
                value={times.to} onChange={e => setTimes({...times, to: e.target.value})} />
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <input className="w-full border rounded p-2 text-sm" placeholder="Tỉnh / Thành phố" 
            value={locationState.province} onChange={e => setLocationState({...locationState, province: e.target.value})} />
          <input className="w-full border rounded p-2 text-sm" placeholder="Quận / Huyện" 
            value={locationState.district} onChange={e => setLocationState({...locationState, district: e.target.value})} />
          <input className="w-full border rounded p-2 text-sm" placeholder="Phường / Xã / Chi tiết" 
            value={locationState.specific} onChange={e => setLocationState({...locationState, specific: e.target.value})} />
        </div>
...
<option key={dt.id} value={dt.id}>{dt.name} ({dt.category})</option>
          ...
          <div>
            <label className="text-xs text-gray-500 mb-1 block">Chọn Thuộc tính</label>
            <div className="flex flex-wrap gap-2 h-24 overflow-y-auto border rounded p-2 bg-gray-50">
              {properties.map((p: any) => (
                <label key={p.id} className="flex items-center gap-1 text-xs bg-white px-2 py-1 rounded border cursor-pointer hover:bg-blue-50 transition-colors">
                  <input type="checkbox" 
      ...
      <div className="flex justify-between items-center">
        <div className="text-sm text-gray-500 italic">
          {pivotedData.length > 0 && `Tìm thấy ${pivotedData.length} dòng dữ liệu.`}
        </div>
        <div className="flex gap-3">
            <button onClick={handleExportCSV} disabled={pivotedData.length === 0}
            ...
            <button onClick={handleSearch} disabled={loading}
                ...
                {loading ? "Đang xử lý..." : <><Search className="w-5 h-5" /> Truy vấn</>}
            </button>
        </div>
      </div>
        ...
        <div className="overflow-x-auto max-h-[600px]">
          <table className="w-full text-sm text-left border-collapse">
            <thead className="bg-gray-100 text-gray-700 sticky top-0 z-10">
              <tr>
                <th className="p-3 border-b border-r sticky left-0 bg-gray-100 min-w-[150px]">Thời gian</th>
                <th className="p-3 border-b border-r min-w-[150px]">Thiết bị</th>
                <th className="p-3 border-b border-r min-w-[300px]">Vị trí chi tiết</th>
                
                {dynamicColumns.map(col => (
<th key={col} className="p-3 border-b border-r text-center bg-blue-50 text-blue-800 font-semibold min-w-[100px]">
                        {col}
                    </th>
                ))}
            ...
            <tbody className="divide-y">
              {pivotedData.length === 0 ? (
                <tr><td colSpan={4 + dynamicColumns.length} className="p-12 text-center text-gray-400">Không có dữ liệu hiển thị</td></tr>
              ) : (
                pivotedData.map((row) => (
                  <tr key={row.uniqueKey} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="p-3 border-r font-medium text-blue-600">{row.deviceName}</td>
                    
                    <td className="p-3 border-r text-gray-600 whitespace-normal break-words" title={row.location}>
                        {row.location}
                    </td>

                    {dynamicColumns.map(col => (
                        <td key={col} className="p-3 border-r text-center font-semibold text-gray-700">
                    ...
                    <td className="p-3 text-center">
                      <span className={`px-2 py-1 rounded text-xs font-bold shadow-sm border ${
                        row.label === "CRITICAL" ? "bg-red-50 text-red-600 border-red-200" :
                        row.label === "WARNING" ? "bg-orange-50 text-orange-600 border-orange-200" :
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y">
              {historyList.length === 0 ? (
                <tr><td colSpan={4} className="p-6 text-center text-gray-500">Chưa có lịch sử xuất file</td></tr>
              ) : (
                historyList.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50">
                    ...
                    <td className="p-3 font-medium text-green-700">{item.filterName}</td>
